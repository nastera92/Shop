...
class SongsTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.buySongButton?.setTitle(String(song.price) + "$", for: .normal)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "playMusic"{
            if let indexPaths = tableView?.indexPathForSelectedRow{
                let destinationController = segue.destination as! MusicPlayerViewController
                let song = songSet[indexPaths.row]
